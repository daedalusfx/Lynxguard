...
// cache only in RAM (برای history_data فقط)
const historyCache = {}; // key = `${symbol}_${period}`
...
app.post('/data', (req, res) => {
    const { symbol, type, period, data } = req.body;

    if (!symbol || !type) {
        return res.status(400).json({ error: 'Missing symbol or type' });
    }

    if (type === 'history_data') {
        const key = `${symbol}_${period}`;
        historyCache[key] = data; // فقط در RAM
        console.log(`📦 Cached ${data.length} bars for ${key}`);
        return res.json({ status: 'cached' });

    } else if (type === 'update_data') {
        broadcast({ type: 'update_data', payload: req.body });
        return res.json({ status: 'broadcasted' });

    } else if (type === 'chart_reset') {
        broadcast({ type: 'chart_reset', symbol, period, sourceIdentifier: "reference_lmax" });
        return res.json({ status: 'reset_sent' });
...
app.get('/history', (req, res) => {
    ...
    if (data) {
        return res.json({ type: 'history_data', data });
    } else {
        return res.status(404).json({ error: 'No historical data in memory' });
    }
});
